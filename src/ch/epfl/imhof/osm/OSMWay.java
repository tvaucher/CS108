...
public final class OSMWay extends OSMEntity {
    ...
    public OSMWay(long id, List<OSMNode> nodes, Attributes attributes) throws IllegalArgumentException {
        super(id, attributes);
        if (nodes.size() < 2) throw new IllegalArgumentException("List of node must contain at least 2 elements");
    ...
    public List<OSMNode> nonRepeatingNodes() {
        if (isClosed()) {
            return Collections.unmodifiableList(new ArrayList<>(nodes.subList(0, nodes.size()-1)));
    ...
    public static final class Builder extends OSMEntity.Builder {
        ...
        public OSMWay build() throws IllegalStateException {
            if (isIncomplete()) throw new IllegalStateException("Cannot build Incomplete Way");
